...
def plot_cluster(cl):
	...
	plot.title('Number of servers= {}, Avg load= {}'.format(cl.num_server, avg_load))
	plot.gcf().set_size_inches(6, 4)
...
def sim_wConstantEndToEndDelay():
	...
	slowdown_rv = DiscreteRV(p_l=[1], v_l=[1]) # Dolly() # DiscreteRV(p_l=[0.9, 0.1], v_l=[1, 2])
	cl_id = 'cl'
	cl = Cluster(cl_id, env, slowdown_rv, NUM_SERVER)
	cid = 'c'
	c = Client(cid, env, cl_id, serv_time_rv, num_req_to_recv=1000*NUM_SERVER)
	cl.reg(cid)
	n = Net_wConstantDelay('n', env, [cl, c], delay=0.1) # 2*serv_time_rv.mean()
